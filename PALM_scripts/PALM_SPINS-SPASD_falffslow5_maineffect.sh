...
#SBATCH --output=/projects/sbagheri/PALM_conmat_bothsoccog_falff_main_effect_slow5_%j.txt  # Standard output and error log
...
filename="$(find $DIR/filelists/*slow5*txt -type f | head -n $SLURM_ARRAY_TASK_ID | tail -n 1)"

# Modify the output file name for consistency
truncname="$(echo $(basename `echo "$filename"`) | sed 's/filepaths/PALM_conmat_bothsoccog_falff_main_effect_slow5/;s/.txt//')" 

# Set the path for the design and contrast matrices for PALM
desmat="$DIR/con_design_bothsoccog/desmat1.csv" 
conmat="$DIR/con_design_bothsoccog/conmat1.csv" 

# Define the output directory for PALM
outdir="${DIR}/PALM_result_bothsoccog_13Dec2023/$truncname"
...
runpalm() {
    ...
    wb_command -cifti-create-dense-from-template ${infile} results_cort_tfce_tstat_fwep_c13.dscalar.nii -volume-all results_sub_tfce_tstat_fwep_c13.nii -metric CORTEX_LEFT results_L_cort_tfce_tstat_fwep_c13.gii -metric CORTEX_RIGHT results_R_cort_tfce_tstat_fwep_c13.gii
    wb_command -cifti-create-dense-from-template ${infile} results_cort_tfce_tstat_fwep_c14.dscalar.nii -volume-all results_sub_tfce_tstat_fwep_c14.nii -metric CORTEX_LEFT results_L_cort_tfce_tstat_fwep_c14.gii -metric CORTEX_RIGHT results_R_cort_tfce_tstat_fwep_c14.gii
    wb_command -cifti-create-dense-from-template ${infile} results_cort_tfce_tstat_fwep_c15.dscalar.nii -volume-all results_sub_tfce_tstat_fwep_c15.nii -metric CORTEX_LEFT results_L_cort_tfce_tstat_fwep_c15.gii -metric CORTEX_RIGHT results_R_cort_tfce_tstat_fwep_c15.gii
    wb_command -cifti-create-dense-from-template ${infile} results_cort_tfce_tstat_fwep_c16.dscalar.nii -volume-all results_sub_tfce_tstat_fwep_c16.nii -metric CORTEX_LEFT results_L_cort_tfce_tstat_fwep_c16.gii -metric CORTEX_RIGHT results_R_cort_tfce_tstat_fwep_c16.gii
...
# Find differences
    ...
    wb_command -cifti-math '(x-y)' ${fname}_tstat_fwep_c1314.dscalar.nii -var x results_cort_tfce_tstat_fwep_c13.dscalar.nii -var y results_cort_tfce_tstat_fwep_c14.dscalar.nii
    wb_command -cifti-math '(x-y)' ${fname}_tstat_fwep_c1516.dscalar.nii -var x results_cort_tfce_tstat_fwep_c15.dscalar.nii -var y results_cort_tfce_tstat_fwep_c16.dscalar.nii
}
