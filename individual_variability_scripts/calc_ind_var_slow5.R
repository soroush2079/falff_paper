...
# for slow 5 fALFF as a measure of quantifying individual 
...
list_of_files <- list.files(path = "/mnt/tigrlab/scratch//jgallucci/for_soroush/CTRL_5_UPDATED_DEC13///" ,recursive = TRUE,
                            pattern = "\\.txt$", 
                            full.names = TRUE)

# 186:253 which includes the ASD participants.
list_of_files2<- list.files(path = "/mnt/tigrlab/scratch/jgallucci/for_soroush/ASD_5_UPDATED_DEC13///",recursive = TRUE,
                            pattern = "\\.txt$", 
                            full.names = TRUE)

# 254:495 which includes the SSD participants.
list_of_files3<- list.files(path = "/mnt/tigrlab/scratch//jgallucci/for_soroush/SCZ_5_UPDATED_DEC13///",recursive = TRUE,
                            pattern = "\\.txt$", 
                            full.names = TRUE)
...
MCD_slow5 <- cbind(ALL_files, distance)
write.csv(MCD_slow5, "ind_var_5_updated_DEC13.csv")
